...
from app.config import API_KEY, DATABASE_DIR, MODEL_CHAT, MODEL_EMBEDDING, NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD
import logging
...
class UnifiedRAGChatBot:
    ...
    def _resolve_process_candidates(self, term: str, limit: int = 5):
        ...
        try:
            ...
            print(f"❌ resolve candidates error: {e}")
            return []
    ...
    def get_graph_visual_data(self, user_input, related_pids=[], full_text_map={}, path_nodes=[]):
        ...
        try:
            ...
            print(f"❌ Dynamic Error: {e}"); return {"nodes": [], "links": []}
    ...
    def chat(self, user_input, chat_history=[]):
        ...
        if a_term and b_term:
            print(f"✂️ 질문 분리 분석: [{a_term}] -> [{b_term}]")
            # 각 용어에 대해 정확한 후보 선출
        ...
        if current_route in ("graph", "both") and len(vector_items) >= 2 and mode != "none":
            ...
            if mode == "conditional":
                ...
                if must_pass_term and self.process_col:
                    try:
                        ...
                        print(f"⚠️ mustPass resolve error: {e}")
            ...
            try:
                ...
                import traceback
                traceback.print_exc()
                print(f"❌ text2cypher path error: {e}")
