...
from app.config import DATASET_DIR, DATABASE_DIR, NEO4J_URI, NEO4J_USERNAME, NEO4J_PASSWORD
from app.utils import get_embedding, chunk_text, extract_text_from_pptx, parse_markdown_structure
...
class DBBuilder:
    ...
    def _build_common(self, source_folder, db_path, collection_name, file_ext, is_markdown=False, limit=None):
        print(f"\n🏗️  DB Build Start: {collection_name}")
        ...
        if not file_list:
            print(f"⚠️  No {file_ext} files found in {source_folder}")
            return
        ...
        print(f"\n✅ Build Complete! Total Chunks: {total_chunks_count}")
